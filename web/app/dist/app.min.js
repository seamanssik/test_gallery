...
var $, Album, AlbumCollection, App, AppController, AppRouter, Backbone, FeedCollectionView, FeedItemView, FeedTemplateBasic;
...
FeedTemplateBasic = '<div> <b><a href="#album/<%= id %>"><%= name %></a></b> <br/> </div>';
...
FeedItemView = Backbone.Marionette.ItemView.extend({
  ...
  initialize: function(options) {
    this.model.listenTo(this, 'sync', this.render);
    if (options.itemId) {
...
FeedCollectionView = Backbone.Marionette.CollectionView.extend({
  ...
  showCollection: function() {
    ...
    return this.collection.each(function(item, index) {
      if (self.filterString !== null && (self.filterString.length === 0 || !self.filter(item))) {
        return;
      }
      return self.addItemView(item, itemView, index);
    });
  },
...
AppController = Backbone.Marionette.Controller.extend({
  feed: function() {
    return App.mainRegion.show(new FeedCollectionView());
  },
  AlbumDetails: function(albumId) {
    return App.mainRegion.show(new FeedItemView({
      displayMode: 'full',
      itemId: albumId
    }));
  }
...
},{"backbone":4,"backbone.marionette":2,"jquery":5,"underscore":6}]},{},[7])
